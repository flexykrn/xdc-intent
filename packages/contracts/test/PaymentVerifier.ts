  ...
  describe("verifyPayment", function () {
    it("Should verify valid payment", async function () {
      const { proof, signature } = await createAndSignProof(
        INTENT_ID, solver.address, TOKEN, AMOUNT, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
    ...
    it("Should emit TotalFeesUpdated and TotalIntentsUpdated", async function () {
      ...
      const { proof, signature } = await createAndSignProof(
        newIntentId, solver.address, TOKEN, ethers.parseEther("100"), ethers.parseEther("0.1"), await getExpiry(), 31337, signer
      );
    ...
    it("Should revert with zero solver", async function () {
      const { proof, signature } = await createAndSignProof(
        INTENT_ID, ethers.ZeroAddress, TOKEN, AMOUNT, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
    ...
    it("Should revert with zero token", async function () {
      const { proof, signature } = await createAndSignProof(
        INTENT_ID, solver.address, ethers.ZeroAddress, AMOUNT, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
    ...
    it("Should revert with zero amount", async function () {
      const { proof, signature } = await createAndSignProof(
        INTENT_ID, solver.address, TOKEN, 0, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
    ...
    it("Should revert with wrong chain ID", async function () {
      const { proof, signature } = await createAndSignProof(
        INTENT_ID, solver.address, TOKEN, AMOUNT, PROTOCOL_FEE, await getExpiry(), 999, signer
      );
  ...
  describe("verifyPaymentBatch", function () {
    it("Should verify batch of payments", async function () {
      ...
      const { proof: proof1, signature: sig1 } = await createAndSignProof(
        intentId1, solver.address, TOKEN, AMOUNT, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
      const { proof: proof2, signature: sig2 } = await createAndSignProof(
        intentId2, solver.address, TOKEN, AMOUNT, PROTOCOL_FEE, await getExpiry(), 31337, signer
      );
